...
<br>
        ...
        <form method="post" action='<?php echo($_SERVER["PHP_SELF"]) ?>'>
            ...
            <input type="submit" name="invio" value="Elimina">
        </form>
    </body>
</html>
...
<?php
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $listsql = "DELETE FROM artisti WHERE Nome = '$nome' AND Cognome = '$cognome' ";

        if (mysqli_query($conn, $listsql)) {
            echo "<h3 style='color:#ffbf00; text-align:center;'>Artista eliminato con successo</h3>";
        } else {
            echo "<h3 style='color:red; text-align:center;'>Errore" . mysqli_error($conn) . "</h3>";
        }
    }
?>